<script lang="ts">
    ...
    async function getReviews() {
        let response = await requestToApi(
            "GET",
            `SmartEval/Reviews/MadeAboutEmployeesOfSuperior?page=${reviewsPage}&pageSize=${reviewsSize}`,
        );
        if (response.statusCode === 200) {
            reviews = response.data;
            reviewsTotal = response.totalCount;
            reviewsTotal = response.totalCount;
            firstElement = Math.min(
                (reviewsPage - 1) * reviewsSize + 1,
                reviewsTotal,
            );
    ...
    async function getEmployeesOfSuperiorInReview() {
        let response = await requestToApi(
            "GET",
            `SmartEval/Reviews/${reviewsChoosen.reviewId}/EmployeesOfSuperior`,
        );
        if (response.statusCode === 200) {
    ...
    async function getSubmissionsFromReview() {
        let response = await requestToApi(
            "GET",
            `SmartEval/Submissions/MadeAboutEmployee?reviewId=${reviewsChoosen.reviewId}&employeeId=${employeesChoosen.employeeId}`,
        );
        if (response.statusCode === 200) {
            submissionsOfEvaluations = response.data.evaluations;
        }
    }
    ...
    async function getTablePerformanceEmployee() {
        let response = await requestToApi(
            "GET",
            `SmartEval/Performance/Employees?reviewId=${reviewsChoosen.reviewId}&employeeId=${employeesChoosen.employeeId}&language=${lang}`,
        );
        if (response.statusCode === 200) {
            tableData = response.data;
            tableData.categories = tableData.categories.map((cat: any) => {
                cat.isOpen = false;
                return cat;
            });
        }
    ...
    async function nextPage() {
        if (reviewsChoosen == null) {
            toast.error($LL.TeamPerformance.ToastSelectReviewError());
            return;
        }
        loaded = false;
    ...
    function changeReviewPage(change: string) {
        if (
            change == "increment" &&
            reviewsPage < Math.ceil(reviewsTotal / reviewsSize)
        ) {
    ...
    {#if page == 1}
        <div class="flex flex-col gap-y-5">
            <div class="flex flex-col">
                <span
                    class="font-semibold text-center lg:text-left text-xl text-black"
                    >{$LL.TeamPerformance.Title()}</span
                >
                <span class="hidden md:inline text-sm text-gray-400"
                    >{$LL.TeamPerformance.Description()}</span
                >
            </div>
            {#if loaded && reviews.length > 0}
                <div class="flex flex-col">
                    {#each reviews as review, index}
                        <label
                            class="border-b cursor-pointer flex items-start justify-between px-4 py-2 border-gray-300 hover:bg-gray-100 {index ==
                            0
                                ? 'rounded-t'
                                : ''}"
                        >
                    ...
                    <div class="flex items-center justify-between px-5 py-2">
                        <span class="text-sm"
                            >{$LL.TeamPerformance.ShowingItemsLabel({
                                firstElement,
                                pageSize: lastElement,
                                total: reviewsTotal,
                            })}</span
                        >
                        {#if reviewsTotal > reviewsSize}
                            <div class="flex gap-x-2">
                                <button
                                    on:click={() =>
                                        changeReviewPage("decrement")}
                                    class="border mx-auto rounded shadow border-gray-300 hover:bg-gray-100"
                                >
                                    <svelte:component this={ChevronLeftIcon} />
                                </button>
                                <button
                                    on:click={() =>
                                        changeReviewPage("increment")}
                                    class="border mx-auto rounded shadow border-gray-300 hover:bg-gray-100"
                                >
                ...
                <button
                    on:click={nextPage}
                    class="font-semibold mx-auto px-2 py-1 rounded bg-blue-500 hover:bg-blue-600 text-white"
                    >{$LL.TeamPerformance.Next()}</button
                >
            {:else if loaded}
    ...
    {:else if page == 2}
        {#if loaded}
            <span class="font-semibold text-xl text-black"
                >{reviewsChoosen.title}</span
            >
            <div class="flex flex-col gap-y-1">
                <div class="flex flex-col">
                    <p class="font-semibold text-base text-black">
                        {$LL.TeamPerformance.ChooseEmployeeTitle()}
                    </p>
                    <p class="text-xs text-gray-400">
                        {$LL.TeamPerformance.ChooseEmployeeDesc()}
                    </p>
                </div>
                <select
                    bind:value={employeesChoosen}
                    on:change={() => {
                        getTablePerformanceEmployee();
                        getSubmissionsFromReview();
                    }}
                    class="border focus:outline-none my-2 p-2 rounded text-sm border-gray-300 bg-gray-100"
                >
            ...
            <div class="border-b flex border-gray-300">
                <button
                    ...
                    class="font-medium px-4 py-2 text-sm {selectTab == 1
                        ? 'border-b-2 border-blue-500 text-gray-800'
                        : 'hover:bg-gray-100 text-gray-400'}"
                    >{$LL.TeamPerformance.EmployeePerformance()}</button
                >
                <button
                    on:click={() => (selectTab = 2)}
                    class="font-medium px-4 py-2 text-sm {selectTab == 2
                        ? 'border-b-2 border-blue-500 text-gray-800'
                        : 'hover:bg-gray-100 text-gray-400'}"
                    >{$LL.TeamPerformance.SubmissionDetails()}</button
                >
            </div>
            {#if selectTab == 1}
                <div class="flex flex-col px-4 py-2">
                    <div
                        class="flex font-medium h-10 items-center bg-blue-400 text-white"
                    >
                        <span class="flex flex-grow"></span>
                        {#each tableData.total as item}
                            <span class="w-20 text-center text-sm"
                                >{getEvaluationTypeText(item.type)}</span
                            >
                    ...
                    {#each tableData.categories as category, index}
                        ...
                        >
                            <span
                                ...
                                >{index + 1}. {category.title}</span
                            >
                            {#each category.averagesWithPercentages as item}
                                <span
                                    class="flex-shrink-0 font-medium text-center text-sm w-20"
                                    >{item.value ?? "-"}</span
                                >
                                <span
                                    class="flex-shrink-0 font-medium text-center text-sm w-10"
                                    >{item.percentageCategory}%</span
                                >
                            {/each}
                        </button>
                        <div>
