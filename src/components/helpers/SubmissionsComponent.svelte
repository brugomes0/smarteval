<script lang="ts">
    ...
    import { ChevronLeftIcon, ChevronRightIcon, CircleCheckIcon, CircleXIcon, FolderIcon, SearchIcon, Trash2Icon, UserCircleIcon, XIcon } from "lucide-svelte"
    import ModalActionReviewComponent from "./ModalActionReviewComponent.svelte";
...
{#if modalDeleteIsOpen}
    <ModalActionReviewComponent on:save={() => deleteSubmission(deleteSubmissionId)}>
        <div class="flex items-center justify-between" slot="header">
            <span class="font-medium text-base text-gray-800">Remover submissão</span>
        ...
        <div class="flex flex-col gap-y-2" slot="content">
            <span class="text-sm text-gray-400">Tem a certeza que pretende remover a submissão? Este passo é irreversível.</span>
        </div>
    ...
    <div class="border flex flex-col mx-5 my-2 rounded border-gray-300">
        <div class="border-b rounded-t bg-gray-100 border-gray-300">
            <div class="border-b flex gap-x-2 h-12 items-center justify-end px-3 border-gray-300">
                ...
                <button class="font-medium px-2 py-1 rounded text-sm bg-blue-500 hover:bg-blue-600 text-white">{$LL.SubmissionModal.CreateSubmission()}</button>
            </div>
        ...
        <div class="flex flex-col h-[200px]">
            ...
            {:else}
                {#if submissions.length > 0}
                    {#each submissions as submission, index}
                        <div class="flex h-10 items-center justify-between px-2 py-1 {index % 2 === 0 ? 'bg-gray-50' : 'bg-white'} {submissions.length == index + 1 ? 'rounded-b' : ''}">
                            <p class="flex gap-x-[5px] items-center text-xs w-[120px] xl:w-[200px] 2xl:w-[300px]">
                                <svelte:component this={UserCircleIcon} class="w-5 h-5" />
                            ...
                            <p class="flex gap-x-[5px] items-center text-xs w-[120px] xl:w-[200px] 2xl:w-[300px]">
                                <svelte:component this={submission.evaluatedEmployee ? UserCircleIcon : FolderIcon} class="w-5 h-5" />
                    ...
                    {/each}
                {:else if error}
                    <span class="border-2 px-4 py-2 rounded bg-red-500 border-red-600">{$LL.SubmissionModal.Error()}</span>
                {:else}
