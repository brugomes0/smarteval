<script lang="ts">
    ...
    import { ChevronLeftCircleIcon, ChevronLeftIcon, ChevronLeftSquareIcon, ChevronRightCircleIcon, ChevronRightSquareIcon } from "lucide-svelte"
    ...
    // let observer: IntersectionObserver
    // let selectedCategory: CategoryInfoData = { categoryId: "", title: "", description: "" };
    // let selectedEmployee: InfoEmployeeData
    // let value: number = 0
    ...
    //     const response = await requestToApi(
    //         "GET",
    //         `SmartEval/Categories/${category.categoryId}/Competency?employeeId=${selectedEmployee.employeeId}&language=${lang}`,
    //     );
    //     if (response.statusCode === 200) {
    //         labels = response.data.reviewTitles.map((r: any) => {
...
<div class="flex flex-col gap-y-5">
    ...
    {#if page == 1}
        <div in:fly={{ delay: 200, duration: 200, x: -50 }} class="flex flex-col gap-y-2">
            ...
            <button on:click={() => changePage("next")} class="font-semibold mt-5 mx-auto px-2 py-1 rounded bg-blue-500 hover:bg-blue-600 text-white">
                {$LL.Competencies.Next()}
            </button>
        </div>
    {:else if page == 2}
